...
export async function POST(request: Request) {
  try {
    ...
    // Step 2: Check if we have a session (email confirmation status)
    if (!authData.session) {
      debugInfo.push({
        step: 2,
        action: 'No session - email confirmation required',
        success: true,
        data: {
          message: 'User needs to confirm email before profile operations',
        },
      });

      return NextResponse.json({
        success: true,
        message: 'Signup successful - email confirmation required',
        user: {
          id: authData.user.id,
          email: authData.user.email,
    ...
    // Step 3: We have a session, proceed with profile operations
    debugInfo.push({
      step: 3,
      action: 'Session available - proceeding with profile operations',
    });
    ...
    debugInfo.push({ step: 5, action: 'Checking if profile exists' });
    ...
    if (profileError) {
      ...
      debugInfo.push({ step: 6, action: 'Creating profile manually' });
    ...
    // Update profile
    debugInfo.push({ step: 6, action: 'Updating profile with flat number' });
